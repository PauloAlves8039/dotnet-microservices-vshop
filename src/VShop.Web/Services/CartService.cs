...
namespace VShop.Web.Services
{
    public class CartService : ICartService
    {
        ...
        public async Task<CartViewModel> AddItemToCartAsync(CartViewModel cartViewModel, string token)
        {
            ...
            StringContent content = new StringContent(JsonSerializer.Serialize(cartViewModel), Encoding.UTF8, "application/json");
            ...
            using (var response = await client.PostAsync($"{apiEndpoint}/addcart/", content))
            {
                ...
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
                    cartViewModel = await JsonSerializer.DeserializeAsync<CartViewModel>(apiResponse, _options);
                }
                else
                {
                    return null;
        ...
        public async Task<CartViewModel> UpdateCartAsync(CartViewModel cartViewModel, string token)
        {
            ...
            using (var response = await client.PutAsJsonAsync($"{apiEndpoint}/updatecart/", cartViewModel))
            {
                if (response.IsSuccessStatusCode)
                {
                    var apiResponse = await response.Content.ReadAsStreamAsync();
